...
for f in out/std/eth/*_std.parquet; do
  ...
  python3 processing/label_merge.py \
    ...
    --out "out/labeled/eth/${base}_labeled.parquet" || echo "[warn] label_merge failed: $f" | tee -a out/logs/process_all.log
done
...
for f in out/labeled/eth/*_labeled.parquet; do
  ...
  python3 processing/classify_context.py \
    --input "$f" \
    --out "out/classified/eth/${base}_classified.parquet" || echo "[warn] classify failed: $f" | tee -a out/logs/process_all.log
done
...
echo "[done] process_all finished"
